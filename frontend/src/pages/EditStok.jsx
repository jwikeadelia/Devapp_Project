...
export default function EditStok({ stok, handleUpdate }) {
  const [showModal, setShowModal] = useState(false);
  const [id_barang, setIDBarang] = useState(stok?.id_barang || "");
  const [tanggal, setTanggal] = useState(stok?.tanggal || "");
  const [jumlah, setJumlah] = useState(stok?.jumlah || "");

  const openModal = () => {
    setIDBarang(stok.id_barang);
    setTanggal(stok.tanggal);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    fetch(`http://localhost:5000/api/barangmasuks/${stok.id}`, {
      method: "PUT",
