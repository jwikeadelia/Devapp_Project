...
export default function Transaksi() {
  ...
  // Ambil semua barang
  const fetchBarangs = () => {
    fetch("http://localhost:5000/api/barangkeluars", { credentials: "include" })
  ...
  // Hapus
  const hapus = (id) => {
    fetch(`http://localhost:5000/api/barangkeluars/${id}`, {
      method: "DELETE",
      credentials: "include",
    })
      .then(() => fetchBarangs())
  ...
  const handleSearchInput = (e) => {
    ...
    // jika ada query → cari
    fetch(`http://localhost:5000/api/barangkeluars/search?q=${value}`, {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => setBarangs(data))
